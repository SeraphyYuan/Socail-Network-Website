...
public class Servletmessage extends HttpServlet {
	...
	response.setContentType("text/html; charset=gbk"); 
                    ...
                    while(rs.next())
                    {
                          id_db=rs.getString(1).trim();
                          date_db=rs.getString(2);
                          content_db=rs.getString(3).trim();
                          sender_db=rs.getString(4).trim();
                         receiver_db=rs.getString(5).trim();
                          System.out.println("id_db:" + rs.getString(1).trim()+ "	" );
                          System.out.println("date_db:" + rs.getString(2).trim()+ "	" );
                          System.out.println( content_db );
                          System.out.println(sender_db);
                          System.out.println(receiver_db);
                          
                          request.setAttribute("sender", sender_db);
                          request.setAttribute("content", content_db);
                          request.setAttribute("date", date_db);
                          request.setAttribute("receiver", receiver_db);
